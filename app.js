...
function getWeather(getCityName) {
  const weatherApi = `
  http://api.openweathermap.org/data/2.5/weather?q=${getCityName}&&mode=json&units=metric&APPID=03261ca6fe9eddee4b88dd99f9a16155`;
  window
    .fetch(weatherApi)
    .then((data) => {
      data
        ...
        .then((weather) => {
          ...
          for (let x of weatherData) {
            ...
            <section id="weatherSet">
                            ...
                            <span class="main float-left">${x.description}</span>
